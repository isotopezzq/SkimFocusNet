...
class LSAA(nn.Module):
    def __init__(self, input_dim=1024, hdim1=512):
        ...
        self.onestage=SingleStage(in_feat_dim=input_dim)
...
class SkimBranch(nn.Module):
    def __init__(self, scales, frames):
        super(SkimBranch, self).__init__()
        self.scales = scales
    ...
    def forward(self, xpre):
        ...
        for feature in xpre:
            ...
            # max_feature.append((feature * norm_label).sum(1)) # weighted sum

            # max_feature.append(torch.mean(feature, dim=1)) #average

            max_feature.append(torch.max(feature, dim=1)[0]) #max
        return max_feature, globpred





class SkimFocusNet(nn.Module):
    def __init__(self, config, checkpoint, num_frames, scales, OPEN=False):
        super(SkimFocusNet, self).__init__()
        ...
        self.skimbranch = SkimBranch(self.scales, self.num_frames)
        self.lsaa = LSAA(input_dim=1024)
        self.sims = Similarity_matrix()
        self.conv3x3 = nn.Conv2d(in_channels=4 * len(self.scales),  # num_head*scale_num
                                 out_channels=32,
                                 kernel_size=3,
    ...
    def Feature_Extractor(self, x):
        ...
        for scale in self.scales:
            if scale == 4:
                xp = self.Replication_padding1(x)
                crops = [xp[:, :, i:i + scale, :, :] for i in
                            range(0, self.num_frames - scale + scale // 2 * 2, max(scale // 2, 1))]
            elif scale == 8:
                xp = self.Replication_padding2(x)
                crops = [xp[:, :, i:i + scale, :, :] for i in
                            range(0, self.num_frames - scale + scale // 2 * 2, max(scale // 2, 1))]
            else:
