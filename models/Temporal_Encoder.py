...
class SingleStage(nn.Module):
    def __init__(self, in_feat_dim=512, embed_dim=512,
                 num_head=8, mlp_ratio=8, norm_layer=nn.LayerNorm,
                 num_block=3):
        super().__init__()
        self.Temporal_Merging_Block = Temporal_Merging_Block(kernel_size=3, stride=1, in_chans=in_feat_dim,
                                              embed_dim=embed_dim)
        self.block = nn.ModuleList([GLRBlock(
            dim=embed_dim, num_heads=num_head, mlp_ratio=mlp_ratio,norm_layer=norm_layer)
            for i in range(num_block)])
    ...
    def forward(self, x):
        x = self.Temporal_Merging_Block(x)
